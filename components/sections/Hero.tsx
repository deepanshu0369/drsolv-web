...
type SurveyVideo = {
  title: string;
  /** Full YouTube watch/share URL. Leave empty until the video is published. */
  youtubeUrl: string;
};

// TODO: paste the three YouTube links here once the videos are published.
const surveys: SurveyVideo[] = [
  { title: "If you ever had an accident?", youtubeUrl: "" },
  { title: "What is your biggest fear as a delivery agent?", youtubeUrl: "" },
  { title: "People are suffering with no one to help!", youtubeUrl: "" },
];
...
function VideoCard({ video }: { video: SurveyVideo }) {
  ...
  const href = id ? video.youtubeUrl : undefined;
    ...
    <>
      ...
      >
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="flex h-12 w-12 items-center justify-center rounded-full bg-fg text-white shadow-md transition-transform duration-200 ease-out group-hover:scale-105">
            <Play size={16} strokeWidth={2.5} aria-hidden className="ml-0.5" />
          </div>
        </div>

        {!id && (
          <span className="absolute bottom-2 left-3 text-[10px] uppercase tracking-[0.16em] text-fg-subtle">
            Video coming soon
          </span>
        )}
      </div>
  ...
  const className =
    "solution-card interactive group block overflow-hidden rounded-lg border";
